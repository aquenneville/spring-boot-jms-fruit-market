...
public class MarketOrderMatcher {
	...
	public Transaction updateBalances(Queue<Order> buyQueue, Queue<Order> sellQueue, Order buy, Order sell) {
		...
		if (sell.getQuantity() < buy.getQuantity()) {
			...
			Order newBuyOrder = new Order(buy.getAccount(), buyUUID, buy.getCommodityName(), buy.getPrice(), buy.getQuantity(), buy.getOrderType());
			...
			Order newSellOrder = new Order(sell.getAccount(), sellUUID, sell.getCommodityName(), sell.getPrice(), sell.getQuantity(), sell.getOrderType());
